...
function log.notify(scope, level, verbose, str, ...)
  if not verbose and _G.Mdwa.config ~= nil and not _G.Mdwa.config.debug then
    return
  end
  ...
  vim.notify(
    string.format("[mdwa.nvim@%s] %s", scope, string.format(str, ...)),
    level,
    { title = "mdwa.nvim" }
  )
end
...
function log.warn_deprecation(options)
  ...
  if uses_deprecated_option then
    ...
    log.notify(
      ...
      vim.log.levels.WARN,
      true,
      "use `:h Mdwa.options` to read more."
    )
  end
end
